...
Maximise net APY on the collateral while maintaining strict liquidation safety margins.
Minimise gas and bridge costs through batch execution and efficient routing.
Prevent recursive risk amplification across correlated protocols.
Run fully autonomously via a keeper bot with real‑time risk monitoring and circuit‑breaker logic.
...
Swap the stablecoin for a liquid staking token using a decentralised exchange aggregator.
Restake the token in a restaking protocol to earn additional yield.
Bridge the restaked position to an L2 with higher rewards and lower transaction costs.
Monitor risk continuously; if the health factor approached unsafe levels or if market conditions changed, automatically unwind the position.
...
Oracle sanity checks: The bot compared prices from multiple oracles and halted operations if any feed deviated beyond a set threshold.
Dynamic leverage caps: The maximum permissible leverage adjusted in real‑time based on market volatility and correlation. During turbulence, the bot de‑leveraged proactively.
Emergency circuit breaker: If the health factor (collateral value versus debt) dropped below a critical threshold, the bot triggered a full unwind of all positions and stopped further borrowing until conditions stabilised.
Outcomes
